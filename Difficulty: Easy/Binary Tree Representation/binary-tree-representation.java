...
class Solution{
    ...
    public static void help(Node root,ArrayList<Integer> v,int arr[]){
        if(arr[0]*2+1>=v.size()){
            root.left=null;
            root.right=null;
            return;
        }
        root.left=new Node(v.get(arr[0]*2+1));
        if(arr[0]*2+2>=v.size()){
            root.right=null;
            return;
        }
        root.right=new Node(v.get(arr[0]*2+2));
        help(root.left,v,new int[]{arr[0]*2+1});
        help(root.right,v,new int[]{arr[0]*2+2});
    }
}